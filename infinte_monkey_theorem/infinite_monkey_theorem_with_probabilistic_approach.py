...
while generation <= max_generations:
    ...
    '''fill population selection pool'''
    for i in range(int (population_size / 2)):
        candidate_index = population_selection_index[i][0]
        number_of_copies_in_pool = int (population_selection_index[i][1])
        for _ in range(number_of_copies_in_pool):
            population_selection_pool.append(population[candidate_index])
    selected_parents = []
    for i in range(int (population_size / 2)):
        random_pick = randint(0,len(population_selection_pool)-1)
        selected_parents.append(population_selection_pool[random_pick])
